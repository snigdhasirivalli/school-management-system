...
function Timetable() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const timetableRes = await axios.get("timetable/all/", { headers: { Authorization: `Bearer ${token}` } });
      setTimetableSlots(timetableRes.data);

      if (userProfile.role === "student") {
        // For students, lock the timetable board filters to their own class/section
        setBoardClass(userProfile.school_class || "");
        setBoardSection(userProfile.section || "");
      } else {
        // For admins/teachers, fetch selectors parameters
        ...
        if (classesRes.data.length > 0) {
          ...
          setBoardClass(defaultClass.id);
          setBoardSectionsList(defaultClass.sections || []);
          if (defaultClass.sections?.length > 0) {
            setBoardSection(defaultClass.sections[0].id);
          }
          
          setForm(prev => ({
            ...prev,
            school_class: defaultClass.id,
            section: defaultClass.sections?.length > 0 ? defaultClass.sections[0].id : ""
          }));
      ...
      console.error(err);
      setError("Failed to fetch initial timetable details.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="main-content">
        <div className="page-header">
          <div className="page-title">
            ...
            <p>
              {role === "student"
                ? `Weekly timetable schedule for Class: ${profile?.class_name || "--"} - Section: ${profile?.section_name || "--"}`
                : "Define class schedules and view weekly timetables for different grades"}
            </p>
          </div>
        </div>
        ...
        <div className="dashboard-grid" style={{ gridTemplateColumns: role === "student" ? "1fr" : "1fr 2.5fr", alignItems: "start" }}>
          
          {/* Schedule Form Card (Only for Admin/Teacher) */}
          {role !== "student" && (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            {/* Class Selector Card (Only visible to admin/teacher) */}
            ...
            <div className="content-card">
              ...
              ) : !boardClass || !boardSection ? (
                <p style={{ color: "var(--text-secondary)", textAlign: "center" }}>No class schedule assigned to display.</p>
              ) : (
